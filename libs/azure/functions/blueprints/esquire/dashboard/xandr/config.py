    ...
    "network_analytics": """
        ...
        FROM (
            ...
            FROM OPENROWSET(
                ...
                [day] VARCHAR(MAX),
                [advertiser_id] VARCHAR(MAX),
                [advertiser_name] VARCHAR(MAX),
                [insertion_order_id] VARCHAR(MAX),
                [insertion_order_name] VARCHAR(MAX),
                [line_item_id] VARCHAR(MAX),
                [line_item_name] VARCHAR(MAX),
                [clicks] VARCHAR(MAX),
                [imps] VARCHAR(MAX),
                [cost] VARCHAR(MAX),
                [revenue] VARCHAR(MAX)
    ...
    "network_site_domain_performance": """
        ...
        FROM (
            ...
            FROM OPENROWSET(
                ...
                [day] VARCHAR(MAX),
                [line_item_id] VARCHAR(MAX),
                [site_domain] VARCHAR(MAX),
                [mobile_application_id] VARCHAR(MAX),
                [mobile_application_name] VARCHAR(MAX),
                [supply_type] VARCHAR(MAX),
                [imps] VARCHAR(MAX),
                [clicks] VARCHAR(MAX)
            ) AS [data]
            WHERE line_item_id != 0
    ...
    "buyer_approximate_unique_users_hourly": """
        ...
        FROM (
            ...
            FROM OPENROWSET(
                ...
                [day] VARCHAR(MAX),
                [line_item_id] VARCHAR(MAX),
                [identified_imps] VARCHAR(MAX),
                [unidentified_imps] VARCHAR(MAX),
                [approx_users_count] VARCHAR(MAX),
                [estimated_people_reach] VARCHAR(MAX)
            ) AS [data]
            WHERE line_item_id != 0
        ) AS [data]
        WHERE rank = 1
